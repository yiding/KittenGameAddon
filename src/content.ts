import { ContentScriptRequest, ContentScriptResponse, ContentScriptConfig } from "./messaging";
    ...
    for (let table of document.querySelectorAll("div.res-table")) {
        if (table.classList.contains("craftTable")) {
            for (let row of table.querySelectorAll("div.res-row")) {
                ...
                const amount = row.querySelector("div.res-cell.resource-value")?.innerHTML;
                ...
                let craftLinks  = [];
                for (const craftLink of row.querySelectorAll("div.res-cell.craft-link")) {
            ...
            for (let row of table.querySelectorAll("div.res-row")) {
                ...
                const maxAmount = row.querySelector("div.res-cell.maxRes")?.innerHTML?.slice(1);
                if (name == null || amount == null) {
                    continue;
                }
                resources[name] = {
...
function craft(resource: CraftableResource): boolean {
    ...
    const selected = resource.craftLinks.reduce(
 ...
 */
function craftWhenFull(check?: Resource, craftable?: CraftableResource): boolean {
    if (
        check != null && 
        craftable != null && 
...
async function onMessage(message: ContentScriptRequest): Promise<ContentScriptResponse | void> {
    switch (message.type) {
        case "GetConfig":
